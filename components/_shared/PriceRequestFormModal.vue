  ...
  >
    <template #default="{ close }">
      <transition name="fade" mode="out-in">
        ...
        <div v-else key="form">
          <h4 v-text="$t('catalog.price.get')"></h4>
...
export default {
  ...
  computed: {
    ...mapGetters('catalog', ['priceRequestProduct']),
  ...
  methods: {
    buildFormData() {
      const pageUrl = `${process.env.SITE_URL}${this.$route.fullPath.slice(1)}`
      const formData = new FormData()

      for (const key in this.form) {
        if (key === 'message') {
          const { name, vendor_code } = this.priceRequestProduct
          formData.append(
            key,
            `Товар: ${name}, Артикул: ${vendor_code}; ${this.form[key] ?? ''}`
          )
        } else {
    ...
    onHidden() {
      this.$store.commit('catalog/SET_PRICE_REQUEST_PRODUCT')
